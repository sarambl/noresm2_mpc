...
use phys_grid,      only: get_rlat_all_p  !jks 041119, this function will return an array with column latitudes
...
!   logical               :: change_inp = .false. ! jks 051119. Initialize a boolean to control whether dst_num is changed
   ...
   inp_mult = inp_tag ! jks 191104 this string is to be picked out and replaced with a [0,1] r8
   ...
   do k = top_lev, pver
      do i = 1, ncol
         ...
         if (t(i,k) < tmelt - 5._r8) then
            ...
            ! Arctic specific modification of the dust nuclei number ! jks 041119
            if (rlats(i)*180._r8/3.14159_r8.gt.+66.66667_r8) then
               dst_num = dst_num*inp_mult
            end if
